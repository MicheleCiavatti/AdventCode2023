...
void printSorted() {
    ...
    for (int i = 0; i < set_sorted.n; i++) {
        char *pw;
        switch (set_sorted.hands[i].power) {
            case 0: pw = "HIGH CARD\0"; break;
            case 1: pw = "PAIR\0"; break;
            case 2: pw = "TWO PAIR\0"; break;
            case 3: pw = "THREE OF A KIND\0"; break;
            case 4: pw = "FULL HOUSE\0"; break;
            case 5: pw = "FOUR OF A KIND\0"; break;
            default: pw = "FIVE OF A KIND\0"; 
        }
        printf("HAND: ");
        for (int j = 0; j < CARDS_PER_HAND; j++) {
            printf("%d ", set_sorted.hands[i].cards[j]);
        }
        printf("- VALUE: %lu - POWER: %s - BID: %d\n", set_sorted.hands[i].val, pw, set_sorted.bids[i]);
    }
}
...
bool searchEqualValues(int arr[], int length, int *p_match, int n_valEquals) {
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 0; j < length; j++) {
            if (arr[i] == arr[j] || arr[j] == 1) counter++;
        }
...
bool pairInFullHouse(const int arr[], const int length, const int except) {
    int n = 0;
    int tmp[CARDS_PER_HAND];
    for (int i = 0; i < length; i++) {
        if (arr[i] != 1 && arr[i] != except) {
            tmp[n++] = arr[i];
        }
    }
    for (int i = 0; i < n; i++) {
...
void fullHouse() {
    ...
    for (int i = 0; i < set_input.n; i++) {
        ...
        if (searchEqualValues(set_input.hands[i].cards, CARDS_PER_HAND, &tris, 3)
            && pairInFullHouse(set_input.hands[i].cards, CARDS_PER_HAND, tris)) {
            addHand(i, 4);
        }
    }
    selectionSort(start, set_sorted.n);
        ...
        for (int i = 0; i < CARDS_PER_HAND; i++) {
            if (isdigit(p[i])) set_input.hands[set_input.n].cards[i] = p[i] - '0';
                ...
                switch (p[i]) {
                    ...
                    case 'J': set_input.hands[set_input.n].cards[i] = 1; break;
